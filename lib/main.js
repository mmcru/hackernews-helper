	...
	searchApiRequest = Request({
		...
		onComplete: function(response){
			...
			if (response.json.nbHits > 0) {
				...
				//console.log("sending searchHitsDict");
...
tabs.on('activate', function () {
	console.log(tabs.activeTab.title + " is now active.");
	apiRequest();
    button.disabled = false;
});
...
tabs.on('ready', function() {
	console.log(tabs.activeTab.title + " is now ready");
	apiRequest();
    button.disabled = false;
});


panel.port.on("linkClicked", function(){
	panel.hide();
});


panel.port.on("resizePanel", function(newHeight){
    console.log("resizing to: " + newHeight + " pixels");
